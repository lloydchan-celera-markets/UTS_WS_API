...
public class HkexOapiGateway implements ICmmfListener, ICmmfProcessor, ICmmfAdminService, IOrderGateway
{
	Logger logger = LoggerFactory.getLogger(HkexOapiGateway.class);

	private static final int CMMF_QUERY_ADMIN_SIZE = 37;
	private static final int CMMF_QUERY_COMMAND_SIZE = 5;
	private static final int CMMF_QUERY_PARAM_SIZE = 32;
	...
	private Map<Long, IOrder> m_orderMap = new ConcurrentHashMap<Long, IOrder>();
	...
	public void onResponse(byte[] data)
	{
		...
		synchronized (sync)
		{
			try
			{
				...
				case ORDER_REQUEST:
				{
					CmmfParser.parseCmmfOrderResponse(buf, this);
					break;
				}
				case TRADE_REPORT:
				case BLOCK_TRADE_REPORT:
				{
					CmmfParser.parseCmmfTradeReportResponse(buf, this);
					break;
				}
				case TRADE:
				{
					CmmfParser.parseCmmfTradeResponse(buf, this);
					break;
				}
				case SOD:
				{
				...
				case ADMIN_REQUEST:
				{
					boolean result = CmmfParser.parseCmmfOgAdminResponse(buf);
				...
				case UPDATE_INSTRUMENT:
				{
					...
					CmmfParser.parseCmmfInstrumentUpdateResponse(buf, this);
					break;
				}
				case LAST_PRICE:
				{
					// test_Print_Bytes(data);
					CmmfParser.parseCmmfLastPriceResponse(buf, this);
					break;
				}
				default:
				{
	...
	public void createOrder(IOrder o)
	{
		try
		{
			...
			byte b[] = o.toCmmf();
			byte[] msg = CmmfBuilder.buildMessage(EApp.OMS, EApp.ExchangeConnector, EMessageType.TASK,
					EFoCommand.ORDER_REQUEST, b);
	...
	public void createBlockTradeReport(IBlockTradeReport block)
	{
		...
		try
		{
			...
			int size = buf.limit();
			byte[] msg = CmmfBuilder.buildMessage(EApp.OMS, EApp.ExchangeConnector, EMessageType.TASK,
					EFoCommand.BLOCK_TRADE_REPORT, buf.array(), size);
	...
	public void modifyOrder(IOrder o)
	{
		try
		{
			// IOrder old = m_orderMap.get(o.getOrderId());
			// Double price = o.getPrice();
			// Integer qty = o.getQty();
			// if (old != null) {
			// old.update(price, qty, null, null);
			// old.setStatus(EOrderStatus.PENDING_AMEND);
			// }
			m_orderMap.put(o.getOrderId(), o);
			byte b[] = o.toCmmf();
			byte[] msg = CmmfBuilder.buildMessage(EApp.OMS, EApp.ExchangeConnector, EMessageType.TASK,
	...
	public void cancel(IOrder o, EFoCommand cmd)
	{
		try
		{
			...
			byte b[] = o.toCmmf();
			byte[] msg = CmmfBuilder.buildMessage(EApp.OMS, EApp.ExchangeConnector, EMessageType.TASK, cmd, b);
			CmmfParser.print(msg);
			server.send(msg);
		} catch (Exception e)
		{
	...
	public static void main(String[] args)
	{
		...
		IOrder order = new Order(EOrderStatus.SENT, instr, EOrderType.LIMIT, null, 1l, "", 439d, 100, ESide.BUY, giveup,
				ESessionState.T);
	...
	public void getAllInstrument()
	{
		try
		{
			byte[] msg = CmmfBuilder.buildMessage(EApp.OMS, EApp.ExchangeConnector, EMessageType.QUERY,
					EFoCommand.UPDATE_INSTRUMENT, null);
			for (int i = 0; i < msg.length; i++)
			{
				System.out.print((int) msg[i] + ",");
			}
	...
	public void changePassword(String password, String newPassword)
	{
		try
		{
			...
			buf.put((byte) EAdminAction.CHANGE_PASSWORD.getChar());
			buf.put(padPwd.getBytes());
			buf.put(padNewPwd.getBytes());
			buf.flip();
			byte b[] = buf.array();
			byte[] msg = CmmfBuilder.buildMessage(EApp.OMS, EApp.ExchangeConnector, EMessageType.ADMIN,
					EFoCommand.ADMIN_REQUEST, b);
			for (int i = 0; i < msg.length; i++)
			{
				System.out.print((int) msg[i] + ",");
			}
	...
	public void setReady()
	{
		...
		{
			ByteBuffer buf = ByteBuffer.allocate(CMMF_LOGIN_ADMIN_SIZE);
			buf.put((byte) EAdminAction.SET_READY.getChar());
			buf.flip();
			byte b[] = buf.array();
			byte[] msg = CmmfBuilder.buildMessage(EApp.OMS, EApp.ExchangeConnector, EMessageType.ADMIN,
					EFoCommand.ADMIN_REQUEST, b);
	...
	public void logout()
	{
		...
		{
			ByteBuffer buf = ByteBuffer.allocate(CMMF_LOGIN_ADMIN_SIZE);
			buf.put((byte) EAdminAction.LOGOUT.getChar());
			buf.flip();
			byte b[] = buf.array();
			byte[] msg = CmmfBuilder.buildMessage(EApp.OMS, EApp.ExchangeConnector, EMessageType.ADMIN,
					EFoCommand.ADMIN_REQUEST, b);
	...
	public void login(String password)
	{
		try
		{
			...
			ByteBuffer buf = ByteBuffer.allocate(CMMF_LOGIN_ADMIN_SIZE);
			buf.put((byte) EAdminAction.LOGIN.getChar());
	...
	public void SOD()
	{
		...
		{
			ByteBuffer buf = ByteBuffer.allocate(CMMF_LOGIN_ADMIN_SIZE);
			buf.put((byte) EAdminAction.SOD.getChar());
			buf.flip();
			byte b[] = buf.array();
			byte[] msg = CmmfBuilder.buildMessage(EApp.OMS, EApp.ExchangeConnector, EMessageType.ADMIN,
					EFoCommand.ADMIN_REQUEST, b);
	...
	public void subscribeMarketData()
	{
		...
		{
			ByteBuffer buf = ByteBuffer.allocate(CMMF_LOGIN_ADMIN_SIZE);
			buf.put((byte) EAdminAction.SUBSCRIBE_MARKET_DATA.getChar());
			buf.flip();
			byte b[] = buf.array();
			byte[] msg = CmmfBuilder.buildMessage(EApp.OMS, EApp.ExchangeConnector, EMessageType.ADMIN,
					EFoCommand.ADMIN_REQUEST, b);
	...
	public void unsubscribeMarketData()
	{
		...
		{
			ByteBuffer buf = ByteBuffer.allocate(CMMF_LOGIN_ADMIN_SIZE);
			buf.put((byte) EAdminAction.UNSUBSCRIBE_MARKET_DATA.getChar());
			buf.flip();
			byte b[] = buf.array();
			byte[] msg = CmmfBuilder.buildMessage(EApp.OMS, EApp.ExchangeConnector, EMessageType.ADMIN,
					EFoCommand.ADMIN_REQUEST, b);
	...
	{
		logger.info("command[{}] param[{}]", command, param);
		try
		{
			String padCmd = StringUtils.rightPad(command, CMMF_QUERY_COMMAND_SIZE);
			String padParam = StringUtils.rightPad(param, CMMF_QUERY_PARAM_SIZE);

			ByteBuffer buf = ByteBuffer.allocate(CMMF_QUERY_ADMIN_SIZE);
			buf.put(padCmd.getBytes());
			buf.put(padParam.getBytes());
			buf.flip();
			byte b[] = buf.array();
			byte[] msg = CmmfBuilder.buildMessage(EApp.OMS, EApp.ExchangeConnector, EMessageType.QUERY,
					EFoCommand.OG_QUERY, b);
			// for (int i=0; i<msg.length; i++) {
			// System.out.print((int)msg[i] + ",");
			// }
			server.send(msg);
	...
	@Override
	public void onTradeReport(Long id, EOrderStatus status,
			String reason/* , Integer giveupNum */)
	{
		...
		// }
		m_oms.onCoreTradeReport(id, status, reason, -1);
	}
